...
### 김현영
- 카카오 소셜 로그인
    - 카카오 로그인 API를 사용하여 로그인 엔드포인트 구현
    - 카카오 회원번호, 닉네임, 프로필 이미지 URL을 데이터베이스에 저장
    - 프로필 이미지가 없으면 자체적인 기본 이미지 URL 저장
- 프로필 및 닉네임 등록
    - 신규 회원일 경우 웹 사이트에서 사용할 프로필 이미지와 닉네임을 등록하는 엔드포인트 구현
    - 프로필 이미지 및 닉네임 표시하는 엔드포인트 구현
    - 프로필 이미지는 AWS S3에 카카오 회원번호별로 개별 폴더를 생성하여 업로드하도록 구현
### 박형섭
